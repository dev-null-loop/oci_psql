variable "apply_config" {
  description = "(Optional) Whether a configuration update requires a restart of the database instance or a reload of the configuration. Some configuration changes require a restart of database instances to be applied. Apply config can be passed as `RESTART` or `RELOAD`"
  type        = string
  default     = null
}
...
variable "config_id" {
  description = "(Optional) The [OCID](https://docs.cloud.oracle.com/iaas/Content/General/Concepts/identifiers.htm) of the configuration associated with the database system."
  type        = string
  default     = null
}
...
variable "db_version" {
  description = "(Required) Version of database system software."
  type        = string
}
...
variable "display_name" {
  description = "(Required) (Updatable) A user-friendly display name for the database system. Avoid entering confidential information."
  type        = string
}
...
variable "instance_count" {
  description = "(Optional) Count of database instances nodes to be created in the database system."
  type        = number
  default     = null
}

variable "instance_memory_size_in_gbs" {
  description = "(Optional) (Updatable) The total amount of memory available to each database instance node, in gigabytes."
  type        = number
  default     = null
}

variable "instance_ocpu_count" {
  description = "(Optional) (Updatable) The total number of OCPUs available to each database instance node."
  type        = number
  default     = null
}

variable "instances_details" {
  description = "(Optional) Details of database instances nodes to be created. This parameter is optional. If specified, its size must match `instanceCount`."
  type = list(object({
...
variable "patch_operations" {
  description = "(Optional) (Updatable) For adding and removing from read replica database instances. Please remove the patch_operations after it is applied. Update the instance_count arrodrandly. Cannot be specified when creating the resource."
  type = list(object({
    operation     = string
    selection     = string
...
variable "shape" {
  description = "(Required) (Updatable) The name of the shape for the database instance node. Use the /shapes API for accepted shapes. Example: `VM.Standard.E4.Flex`"
  type        = string
}

variable "source_details" {
  description = "(Optional) The source of the database system."
  type = object({
...
variable "system_type" {
  description = "(Optional) Type of the database system."
  type        = string
  default     = null
}
